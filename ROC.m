addpath 'imgs'
path = 'D:\UCID_color\ucid00';
Roc = zeros(30,1);
format = '.tif';
for filename=100:150
    %%%%%%%%%%%%%%构建水印矩阵%%%%%%%%%%%%%
        %%%
        disp(['File: ' path num2str(filename)  format]);
        I=imread([path num2str(filename)  format]);
...
%                 %JPEG
%                     
                    imwrite(nim,'temp.jpeg','Quality',60);
                    J = imread('temp.jpeg');
%                     J = nim;
%                     J=imresize(nim,0.75);
